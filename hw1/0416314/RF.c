...
#define tree_num 15 //random forest樹的數目
int node_num;
//目的：用測資花朵的四種屬性(descriptive feature)預測花朵種類(class)
//方法：
//1.打亂 data.txt 中的資料（因已照種類排好了）
//2.分割成5組(1 test data、4 training data)
//2-1.所有的training data做隨機取樣，取樣過的依然可以重複取樣，進行tree_num次
//(可略)3.計算現有花瓣種類的熵(entropy of all data)
//4.因4種decriptive feature為連續性資料，故先計算其threshold
//4-1.因每棵樹的樣本不同，故每棵樹都要計算其threshold
//5.分別計算4種屬性的熵(entropy)
...
//9.將test data丟入每棵DTree
//9-1.統計每棵tree的結果，進行vote，選出最終結果
//10.計算準確率
...
/*
	for(int i=0;i<quantity;i++){
		printf("new seq:%d,sepal_l:%f,sepal_w:%f,",i,data[i].sepal_l,data[i].sepal_w );
		printf("petal_l:%f,petal_w:%f,class:%d\n",data[i].petal_l,data[i].petal_w,data[i].class);
	}
*/	
	double t_accuracy=0;
	double accuracy=0;
...
/*transfer the whole data into training data(150->120) with random sample*/
			struct flower* mini_data=(struct flower*)malloc(sizeof(struct flower)*((quantity*(fold-1))/fold));

			while(cnt < data_n){
				seq=rand()%quantity;
				if((i*num<=seq) && ((i+1)*num>seq)) seq=rand()%quantity;
				else{
					//printf("cnt:%d th,chosen seq:%d,class:%d\n",cnt,seq ,data[seq].class);
					mini_data[cnt]=data[seq];
					cnt++;
				}
			}
...
/*build tree*/		
			//struct Node* root;
			root[k]=DTree(mini_data,thre_arr);
...
/*test testdata*/			
		...
		int predict[tree_num]={0};
		int final_pre=0;
		int final_val=0;

		for(int j=0;j<num;j++){
			for(int k=0;k<tree_num;k++){
				predict[k]=traversal(data[i*num+j],root[k]);
				//printf("prediction%dth in %d tree:%d\n",j,k,predict[k] );
				//predict setosa
				switch(predict[k]){
					case 0: t_class[0]++;break;
					case 1: t_class[1]++;break;
					case 2: t_class[2]++;break;
				}	
			}
			//printf("vote: type0=%d, type1=%d, type2=%d\n",t_class[0],t_class[1],t_class[2]);
//t_class[k]>=2,2 depends on tree_num				
			
			final_val=0;
			for(int k=0;k<3;k++){
				if(t_class[k]>final_val) {
					final_pre=k;
					final_val=t_class[k];
					//printf("class:%d,val:%d\n",final_pre,final_val );
				}	
				t_class[k]=0;
			}
			//printf("vote result:class=%d\n",final_pre);
			//predict setosa
			switch(final_pre){
			...
			if(final_pre == data[i*num+j].class){
				switch(final_pre){
