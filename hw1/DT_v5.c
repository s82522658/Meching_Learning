...
/*********************************************************/
/*********************************************************/
/********* last update:10/20,1:15 p.m.********************/
/*  parentropy、totalentropy、threshold 功能完成。待修改。 */
/********************************************************/
int node_num;
struct boundary {
	double bound_min;
...
struct Node* insert_node(struct flower* mini_data,int data_n,struct boundary table[],struct thre thre_arr[][(quantity*(fold-1))/fold]){
	...
	for(int i=0;i<new_bound.r_num;i++){
		...
		//printf("new seq:%d,sepal_l:%f,sepal_w:%f,",i,new_data[i].sepal_l,new_data[i].sepal_w );
		//printf("petal_l:%f,petal_w:%f,class:%d\n",new_data[i].petal_l,new_data[i].petal_w,new_data[i].class);
		//printf("right child %dth,data class:%d\n",i,new_data[i].class);
		//new_data_l[k]=new_data[i];
		//k++;
		//沒有把new_data複製過去的部分歸零，不知道會不會錯？
	}
	for(int i=new_bound.r_num;i<new_bound.r_num+new_bound.l_num;i++){
		//new_data[i]=var.mini_data[i];
		//printf("%dth,data class:%d\n",i,new_data[0].class);
		//new_data_l[k]=revar.mini_data[i];
		new_data_l[k]=revar.mini_data[i];
		//printf("new seq:%d,sepal_l:%f,sepal_w:%f,",k,new_data_l[k].sepal_l,new_data_l[k].sepal_w );
		//printf("petal_l:%f,petal_w:%f,class:%d\n",new_data_l[k].petal_l,new_data_l[k].petal_w,new_data_l[k].class);
		//printf("left child %dth,data class:%d\n",k,new_data_l[k].class);
		k++;
		//沒有把new_data複製過去的部分歸零，不知道會不會錯？
	}/*
	if(new_bound.entropy == 0) {
	//if(entropy == 0) {	
	//不用再分，leaf node全都是同一種class
	//或是，leaf node用剛算好的thres下去分，他的左子樹與右子樹皆會是同一種class
		newnode->class=new_data[0].class;	
		printf("all the same class2\n");
	}*/
	//else{
		printf("new boundary value:%f,l_num:%d,r_num:%d\n",new_bound.value,new_bound.l_num,new_bound.r_num);
		double temp=0.0;
...
//repeat anout 120 times = calculate total 120 features' entropy
		...
		for(int j=0;j<total_num;j++){
			...
			if(!table[i].bound_min&&!table[i].bound_max){
				...
				if(!((((*cnt)==data_n) || ((*cnt)==0)) && entropy!=0)){
					...
					//parentropy(mini_data,int data_n,thre_arr.value,i);
					//printf("seq:%d,cnt:%d,data_n:%d,entropy:%f\n",thre_arr[i][j].seq,(*cnt),data_n,entropy );
	...
	for(int i=0;i<data_n;i++){
		if(i<(min_e.r_num)) printf("seq %d,right child's class:%d\n",i,mini_data[i].class);
		else printf("seq %d,left child's class:%d\n",i,mini_data[i].class);
	}*/
	...
	/*
	for(int i=0;i<data_n;i++){
		revar.mini_data[i]=mini_data[i];
	}*/
	//revar.mini_data=mini_data;
...
void creat_newdata(struct flower* data,int data_n,int feature,int* cnt,double thre_val){
	...
	switch(feature){
		...
		case 3:for(int i=0;i<data_n;i++){
				if(data[i].petal_w<=thre_val){
					mini_data[j]=data[i];
					//printf("right %dth:petal_w=%f\n",j,mini_data[j].petal_w);
					j++;
				}
				else{
					mini_data[(*cnt)+k]=data[i];
					//printf("left %dth:petal_w=%f\n",k,mini_data[(*cnt)+k].petal_w);
					k++;
				}
			}
		break;
	...
	for(int i=0;i<data_n;i++){
		...
		data[i]=mini_data[i];
		//printf("after copy changed data %dth:petal_l=%f\n",i,data[i].petal_l);
...
double parentropy(struct flower* temp,int data_n,int* cnt){
	//int data_n=end-start+1;
	//printf("start:%d,end:%d,data_n:%d,thre_seq:%d\n",start,end,data_n,thre_seq );
	//int *cnt=(int)malloc(sizeof(int));
	//creat_newdata(temp, data_n,feature,cnt);
	//printf("section 4\n");
	...
	for(int i=0;i<3;i++){
		...
		//printf("class_a[%d]/=thre_seq is:%f\n",i,class_a[i]);
		//if(class_a[i] != 0) entropy_a+=class_a[i]*log2(class_a[i]);
		if(class_a[i] != 0) entropy_a+=class_a[i]*(log(class_a[i])/log(2.0));
	}

	//printf("partial entropy_a is:%f\n",entropy_a);
	...
	for(int i=0;i<3;i++){
		...
		//printf("calss_b[i]:%f\n",class_b[i] );
		//printf("class_b[%d]/=thre_seq is:%f\n",i,class_b[i]);
		if(class_b[i] != 0) entropy_b+=class_b[i]*(log(class_b[i])/log(2.0));
		///printf("entropy_b:%f\n",entropy_b );
	}
	//printf("partial entropy_b is:%f\n",entropy_b);
	//printf("ratio_a:%f,ratio_b:%f\n",ratio_a,ratio_b );
...
double totalentropy(struct flower* data,int data_n){
	...
	for(int i=0;i<fold;i++){
		for(int j=0;j<num;j++){
			if(i==test_seq) break;
				...
				if(data[i*num+j].class==0) class[0]++;
				else if(data[i*num+j].class==1) class[1]++;
				else if(data[i*num+j].class==2) class[2]++;
			}
		}
	}*/
	//printf("in totalentropy: data_n:%d\n",data_n);
	for(int i=0;i<data_n;i++){
	...
	for(int i=0;i<3;i++){
		//printf("in totalentropy: class%dth=%f\n",i,class[i]);
		class[i]/=data_n;
		//if(class[i] != 0) entropy+=class[i]*log2(class[i]);
...
//create a structure for storing results from bubblesort
	int data_n=(quantity*(fold-1))/fold;
	/*struct flower temp[(quantity*(fold-1))/fold]={};*/
	/******* important!! reuseable in each k-fold********/
	/*
	struct thre{
		int seq;
		double value;
	};
	struct thre array[4][120]={};
	*/
	/****************************************************/
	...
	for(int i=0;i<data_n-1;i++){
		if(temp[i].class!=temp[i+1].class) {
			...
			//printf("sepal_l:%d:class%d,value:%f,%d+1:class%d,value:%f",i,temp[i].class,temp[i].sepal_l,i,temp[i+1].class,temp[i+1].sepal_l);
			//printf("threshold[0]:%f,at:%d\n",array[0][cnt].value,cnt);
			cnt++;
		}
	}
	...
	for(int i=0;i<data_n-1;i++){
		if(temp[i].class!=temp[i+1].class) {
			...
			array[1][cnt].seq=i+1;
			//printf("threshold[1]:%f,at:%d\n",array[1][cnt].value,cnt);
			//printf("sepal_w:threshold:%f,at:%d\n",array[0][cnt].value,cnt);
			cnt++;
		}
	...
	for(int i=0;i<data_n-1;i++){
		if(temp[i].class!=temp[i+1].class) {
			...
			array[2][cnt].seq=i+1;
			//printf("threshold[2]:%f,at:%d\n",array[2][cnt].value,cnt);
			//printf("petal_l:threshold:%f,at:%d\n",array[0][cnt].value,cnt);
			cnt++;
		}
		//printf("%d th,class:%d\n",i,temp[i].class);
	}
	...
	for(int i=0;i<data_n-1;i++){
		if(temp[i].class!=temp[i+1].class) {
			...
			//printf("petal_w:threshold:%f,at:%d\n",array[0][cnt].value,cnt);
			cnt++;
		}
		//printf("%d th,class:%d\n",i,temp[i].class);
	}
