...
/*store training data into ecoli strcuct*/
 	...
 	for(;i<lSize;i++){
 		if(buffer[i] == ','){
 			comma++;
 			if(comma >= 3 && comma <= 11) data[seq].attribute[comma-3]=atof(buf);
 			for(int j=0;j<15;j++) buf[j]=0;	
 			cnt=0;
 		}
 		else if(buffer[i] == '\n'|| buffer[i] == '\0'){
 			if(buf[0]=='c'&&buf[1]=='p') data[seq].class=0;
 			else if(buf[0]=='i'&&buf[1]=='m'&&buf[2]=='U') data[seq].class=3;
 			else if(buf[0]=='i'&&buf[1]=='m'&&buf[2]=='L') data[seq].class=6;
 			else if(buf[0]=='i'&&buf[1]=='m'&&buf[2]=='S') data[seq].class=7;
 			else if(buf[0]=='i'&&buf[1]=='m') data[seq].class=1;
 			else if(buf[0]=='p'&&buf[1]=='p') data[seq].class=2;
 			else if(buf[0]=='o'&&buf[1]=='m'&&buf[2]=='L') data[seq].class=5;
 			else if(buf[0]=='o'&&buf[1]=='m') data[seq].class=4;
 			data[seq].index=seq;
 			for(int j=0;j<15;j++) buf[j]=0;
 			comma=0;
 			cnt=0;
 	...
 	/*store test data into ecoli strcuct*/
 	comma=0;
 	cnt=0;
 	seq=0;
 	i=0;
 	...
 	for(;i<lSize;i++){
 		if(buffer[i] == ','){
 			comma++;
 			if(comma >= 3 && comma <= 11) testdata[seq].attribute[comma-3]=atof(buf);
 			for(int j=0;j<15;j++) buf[j]=0;	
 			cnt=0;
 		}
 		else if(buffer[i] == '\n'|| buffer[i] == '\0'){
 			if(buf[0]=='c'&&buf[1]=='p') testdata[seq].class=0;
 			else if(buf[0]=='i'&&buf[1]=='m'&&buf[2]=='U') testdata[seq].class=3;
 			else if(buf[0]=='i'&&buf[1]=='m'&&buf[2]=='L') testdata[seq].class=6;
 			else if(buf[0]=='i'&&buf[1]=='m'&&buf[2]=='S') testdata[seq].class=7;
 			else if(buf[0]=='i'&&buf[1]=='m') testdata[seq].class=1;
 			else if(buf[0]=='p'&&buf[1]=='p') testdata[seq].class=2;
 			else if(buf[0]=='o'&&buf[1]=='m'&&buf[2]=='L') testdata[seq].class=5;
 			else if(buf[0]=='o'&&buf[1]=='m') testdata[seq].class=4;
 			for(int j=0;j<15;j++) buf[j]=0;
 			comma=0;
 			cnt=0;
 			seq++;
